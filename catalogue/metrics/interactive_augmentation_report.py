...
def interactive_augmentation_report(
    ...
    """
    This module generates an interactive HTML report featuring a <a href="https://plotly.com/python/sunburst-charts/" target="_blank">Plotly sunburst pie chart</a> visualization to explore imbalances 
    in the dataset based on subgroups defined by sensitive attributes and the target variable. This helps to quickly identify any potential imbalances in the dataset, allowing users to assess fairness and identify areas 
    that may require intervention, such augmentation.

    Furthermore, the report visually presents different augmentation strategies designed to mitigate data imbalances, per sensitive attribute, as an interactive <a href="https://plotly.com/python/bar-charts/." target="_blank">Plotly bar chart</a>. 
    These strategies adjust the distribution of the dataset by oversampling specific subgroups with synthetic samples, ensuring more equitable representation of sensitive attributes and target classes. The following 
    ...
    **"Synthetic Tabular Data Generation for Class Imbalance and Fairness: A Comparative Study"**  
    ...
    # Check if all sensitive attributes are categorical
    non_categorical = [col for col in sensitive if col not in dataset.categorical]
    if non_categorical:
        raise ValueError(
            f"Sensitive attributes {non_categorical} must be categorical. "
    ...
    # Create complete HTML with title, description, and all plots
    complete_html = f"""
    <html>
    <head>
        <title>FairBench Interactive Augmentation Report</title>
        <style>
            ...
            .overview-container {{
                ...
                height: 70vw; /* Ensure it remains a square */
                max-width: 1000px; /* Optional: Limit maximum width */
                max-height: 1000px; /* Optional: Limit maximum height */
    ...
    <body>
        <div class="container">
            <h1>FairBench Interactive Report</h1>
            <div class="description">
                <p>Interactive visualization showing the distribution of data across sensitive attributes 
                ({', '.join(sensitive)}) and target variable.</p>
            </div>
            ...
            <p>Sampling strategies dictate the number of synthetic samples to generate from each subgroup, to create the final augmented dataset. The following strategies are used:</p>
            <ul>
            ...
            <p>The following plots visualize the impact of these strategies on data distribution:</p>
            {''.join([f"""
            ...
            <p>The augmentation ratio, <em>r_aug</em>, represents the percentage of synthetic samples in the final dataset, providing insight into how much the dataset has been augmented.</p>
